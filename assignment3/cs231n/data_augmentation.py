...
def random_crops(X, crop_shape):
  ...
  for i in xrange(N):
    Hloc = randint(0,H-HH) 
    Wloc = randint(0,W-WW)
    out[i] = X[i,:,Hloc:Hloc+HH,Wloc:Wloc+WW]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  #for i in xrange(N):
  #  S = np.zeros([C,H,W])
  #  S.fill(np.random.uniform(low,high))
  #  out[i] = X[i] * S
  
  # Vectorized
  S = (high - low)*np.random.random_sample(N) + low
  out = X * S[:,None,None,None]
...
def random_tint(X, scale=(-10, 10)):
  ...
  #  for d in xrange(C):
  #    T = np.zeros([H,W])
  #    T.fill(np.random.uniform(low,high))
  #    out[i,d,:,:] = X[i,d,:,:] + T
  
  # Vectorized
  T = (high - low) * np.random.random_sample((N,C)) + low
  out = X + T[:,:,None,None]
