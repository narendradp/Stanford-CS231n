...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    hidden_dims.extend([num_classes])
    for idx in xrange(self.num_layers):
      p = 'W' + np.str(idx+1)
      self.params[p] = np.random.normal(0, weight_scale, [hidden_dims[idx], hidden_dims[idx+1]])
      p = 'b' + np.str(idx+1)
    ...
    if self.use_batchnorm:
      self.bn_params = [{'mode': 'train', 'running_mean': 0.0, 'running_var': 0.0} for i in xrange(self.num_layers - 1)]
  ...
  def loss(self, X, y=None):
    ...
    outs.append(X)
    # Forward pass of all affine + relu layers
    for idx in xrange(self.num_layers - 1):
      W = self.params['W' + np.str(idx+1)]
      b = self.params['b' + np.str(idx+1)]
      
      if self.use_batchnorm:
        gamma = self.params['gamma' + np.str(idx+1)]
        beta = self.params['beta' + np.str(idx+1)]
      ...
      if self.use_dropout:
        out, cache = dropout_forward(outs.pop(), self.dropout_param)
        outs.append(out)
        caches.append(cache)    
    
    # Forward pass through the last affine layer
    W = self.params['W' + np.str(self.num_layers)]
    b = self.params['b' + np.str(self.num_layers)]
    scores, cache = affine_forward(outs[-1], W, b)
    ...
    # Compute the gradients through backward pass
    dout, dW, db = affine_backward(dscores, cache)
    grads['W' + np.str(self.num_layers)] = dW
    grads['b' + np.str(self.num_layers)] = db
    
    douts = []
    douts.append(dout)
    for idx in reversed(xrange(self.num_layers - 1)):
      ...
      if self.use_batchnorm:
        ...
        grads['gamma' + np.str(idx+1)] = dgamma
        grads['beta' + np.str(idx+1)] = dbeta
      else:
        dout, dW, db = affine_relu_backward(douts.pop(), caches.pop())
      
      douts.append(dout)
      grads['W' + np.str(idx+1)] = dW
      grads['b' + np.str(idx+1)] = db
    
    # Adding gradients for regularization loss
    for idx in xrange(self.num_layers):
      W = self.params['W' + np.str(idx+1)]
      grads['W' + np.str(idx+1)] += self.reg * W
