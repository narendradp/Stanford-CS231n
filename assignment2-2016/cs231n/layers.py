...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    sample_var = np.zeros(D, dtype=x.dtype)
    sample_mean = np.mean(x, axis=0)
    sample_var = np.std(x, axis=0)**2
    x_norm = (x - sample_mean) / np.sqrt(sample_var + eps)
    out = x_norm * gamma + beta
    cache = (x, gamma, beta, sample_mean, sample_var, eps, momentum)
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  x, gamma, beta, sample_mean, sample_var, eps, momentum = cache
  x_norm = (x - sample_mean) / np.sqrt(sample_var + eps)
  dx_norm = dout * gamma
  dsample_var = np.sum((-0.5) * ((sample_var + eps) ** (-1.5)) * dx_norm * (x - sample_mean), axis=0)
  dsample_mean = np.sum((dx_norm * (-1.0)) / np.sqrt(sample_var + eps), axis=0) + ((dsample_var * (-2.0)) / N) * np.sum(x - sample_mean, axis=0)
  dx = (dx_norm * ( 1.0 / np.sqrt(sample_var + eps))) + ((dsample_var * 2.0 * (x - sample_mean)) / N) + ((1.0 / N) * dsample_mean)
