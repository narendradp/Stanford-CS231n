...
class KNearestNeighbor:
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #######################################################################
      dists[i,:] = np.sum(np.abs(X[i] - self.X_train)**2, axis=1)
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      #########################################################################
      ids = np.argsort(dists[i])
      for j in range(k):
        closest_y = np.append(closest_y, self.y_train[ids[j]])
      #print closest_y
      #########################################################################
      # TODO:                                                                 #
      ...
      max_count = 0
      for j in range(k):
        count = list(closest_y).count(closest_y[j])
        if  count > max_count:
          max_count = count
