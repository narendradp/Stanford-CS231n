...
def rnn_step_backward(dnext_h, cache):
  ...
  dhraw = (1.0 - next_h * next_h) * dnext_h
...
def rnn_backward(dh, cache):
  ...
  dWh = np.zeros([H, H])
  db = np.zeros(H)
  dh_next = dh.transpose(1, 0, 2).copy()
  for t in reversed(xrange(T)):
    cache = caches.pop()
    dh_next[t] += dh_prev
    dx[t], dh_prev, dWx_cur, dWh_cur, db_cur = rnn_step_backward(dh_next[t], cache)
    dWx += dWx_cur
    dWh += dWh_cur
    db += db_cur
...
def word_embedding_forward(x, W):
  ...
  # Naive, un-vectorized implementation
  #N, T = x.shape
  #V, D = W.shape
  #out = np.zeros([N, T, D])
  #for n in xrange(N):
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  next_c_t = np.tanh(next_c)
  dnext_c += (1.0 - next_c_t * next_c_t) * o * dnext_h
  dprev_c = f * dnext_c
...
def lstm_backward(dh, cache):
  ...
  dWh = np.zeros([H, 4*H])
  db = np.zeros(4*H)
  dnext_h = dh.transpose(1, 0, 2).copy()
  dnext_c = np.zeros([N, H])
  for t in reversed(xrange(T)):
    cache = caches.pop()
    dnext_h[t] += dh_prev
    dx[t], dh_prev, dnext_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h[t], dnext_c, cache)
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
